...
```{r}
high_temps <- c(38,36,39,33,28,37,37,32,33,41)
low_temps <- c(24,28,24,19,16,22,29,19,22,32)
```
...
```{r}
celsius <- function(x){
  celsius_temp <- (x-32)/1.8
  return(celsius_temp)
}

high_temps_C <- celsius(high_temps)
high_temps_C

low_temps_C <- celsius(low_temps)
low_temps_C
```
...
```{r}
temps_df <- cbind(high_temps, low_temps, high_temps_C, low_temps_C)
colnames(temps_df) <- c("hot boi F", "cold boi F", "hot boi C", "cold boi C")
```
...
date_obj_today <- ymd(str_today) #converts string of written data to data object
date_obj_today
