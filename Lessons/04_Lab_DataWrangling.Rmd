...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #inclusive range
...
NTL.phys.data.Tues.19901999 <- NTL.phys.data %>% 
  filter(year4 %in% c(1990:1999)) %>%
  filter(lakename == "Tuesday Lake")

#OR

Ex2 <- filter(NTL.phys.data, year4 %in% (1990:1999) & lakename == "Tuesday Lake")
...
NTL.phys.data.Tues.9099.July <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
  filter(Month == 7)
...
NTL.phys.data.Tues.9099.July.surface <- NTL.phys.data.Tues.9099.July %>%
  filter(depth == 0) %>%
  filter(!is.na(temperature_C)) %>% #exclamation point takes the na's out
