...
> Answer: null: The mean AQI is equal to the threshold for "good" AQI index (0-50). alt: The mean AQI is not equal to the threshold for "good" AQI index (0-50).
...
> We reject the null hypothesis that mean AQI is equal to "good" AQI index (p-value < 2.2e-16). Mean AQI is not equal to the (0,50) threshold.
...
shapiro.test(EPAair$Ozone[EPAair$Year == 2019]) #nor normal
...
O3.twosample
O3.twosample$p.value  #0.00773585
...
ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year))) +
  geom_density(alpha=0.5) +
    scale_fill_manual(values=c("red", "yellow")) +
      labs(title="Density Plot: Ozone by Year", fill="Year")
...
ozone.lm <- lm(PM2.5~Ozone, data=EPAair)
summary(ozone.lm) #shows significant relationship between PM2.5 and Ozone
#p-value: < 2.2e-16 (telling us the likelihood that null hyp is true (which is no relationship); is the regression stat. significant)
#Multiple R-squared:  0.1316 (variance from the line; measure of how much variation the regression captures/explains)
...
the_lm7 <- lm(data=EPAair, PM2.5~Ozone + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(the_lm7)
```
