...
#5
ggplot(watersupply.df, aes(x=Date,y=Max_Daily_Use_per_Month, group=1)) + 
  geom_line() + 
...
scrape.it <- function(the.year, pswid){
  ...
  the.website <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=',pswid,'&year=', the.year))
...
ggplot(new.df, aes(x=Date,y=Max_Daily_Use_per_Month, group=1)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
...
ggplot(dur.ash.df, aes(x=Date, y=Max_Daily_Use_per_Month, group=Water_System_Name, color=Water_System_Name)) +
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("Max Daily Withdrawls across the Months for 2015"),
...
ggplot(ash.1019.df,aes(x=Date,y=Max_Daily_Use_per_Month, group=1)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
